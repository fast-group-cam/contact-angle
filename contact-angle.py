...
def droplet_height(waters: np.ndarray) -> float:
    """This function takes in a droplet, assumed to be centered on the z-axis, and finds its height
using the Willard-Chandler instantaneous interface. The inputs are:
...
def contact_angles(waters: np.ndarray, carbons: np.ndarray, cell_params: np.ndarray,
                   N_azimuths: int, rot_angle: float, fig: Figure = None, axes: list[Axes] = None
...
distribution to a PyPlot Figure if provided (as needed for program mode (1)). The inputs are:
    ...
    rot_angle:   The rotational angle to rotate between azimuthal slices.
...
encountered in this frame.
    ...
    droplet_h = droplet_height(waters)

    # Pre-calculate rotation matrix for iterating through azimuthal slices
    rot_matrix = np.array(((cos_deg(rot_angle), -sin_deg(rot_angle), 0.0),
                           (sin_deg(rot_angle), cos_deg(rot_angle),  0.0),
                           (0.0,                0.0,                 1.0)))
    
    # Pre-calculate y-coordinates to search through for each azimuthal slice
    cutoff_y = (0.5 * COARSE_GRAIN_LENGTH * (N_SAMPLE_Y - 1)) + SLICING_WIDTH
    y_coords = np.linspace(0.5 * COARSE_GRAIN_LENGTH * (1 - N_SAMPLE_Y),
                           0.5 * COARSE_GRAIN_LENGTH * (N_SAMPLE_Y - 1), N_SAMPLE_Y)
    ...
    for azi in range(N_azimuths):
        ...
        sliced = waters[waters[:,1] < cutoff_y]
        sliced = sliced[sliced[:,1] > -cutoff_y]
        sliced = sliced[sliced[:,2] < cutoff_z]
        l_waters = sliced[sliced[:,0] < SLICING_WIDTH]
        r_waters = sliced[sliced[:,0] > -SLICING_WIDTH]
        carbon_slice = carbons[carbons[:,1] < cutoff_y]
        carbon_slice = carbon_slice[carbon_slice[:,1] > -cutoff_y]

        # Find first guess of left interface
        ...
        # Use first guess to find reasonable water floor and establish scanning z-coordinates
        lowest_scan_z = (np.min(l_waters[np.abs(l_waters[:,0] - result) < SLICING_WIDTH, 2]) +
                         COARSE_GRAIN_LENGTH)
        z_coords = np.linspace(lowest_scan_z, highest_scan_z, N_SAMPLE_Z)

        # Find left interface
        l_interface = list()
        for y_sample in y_coords:
            for z_sample in z_coords:
                ...
                while (upper - lower) > DISTANCE_TOLERANCE:
                    ...
                    if density > CUTOFF_DENSITY:
                        ...
                        lower = result
                    result = (lower + upper) / 2.0
                l_interface.append([result, y_sample, z_sample])
        
        # Calculate normal vector of left interface
        l_interface = np.array(l_interface)
        l_inter_centroid = np.mean(l_interface, axis=0)
        svd = np.linalg.svd((l_interface - l_inter_centroid).T)[0]
        l_inter_normal = svd[:,-1]
        if l_inter_normal[0] > 0.0:
            l_inter_normal = -l_inter_normal
        
        # Find closest carbon atoms to the left interface, and calculate their normal vector
        l_inter_foot = np.zeros((3,), dtype=float)
        l_inter_foot[0] = l_inter_centroid[0] + (l_inter_centroid[2] * l_inter_normal[2] /
                                                 l_inter_normal[0])
        carbon_dists_sq = np.sum(np.square(carbon_slice - l_inter_foot), axis=-1)
        closest_carbons = carbon_slice[np.argpartition(carbon_dists_sq, 20)[:20]]
        l_carbon_centroid = np.mean(closest_carbons, axis=0)
        svd = np.linalg.svd((closest_carbons - l_carbon_centroid).T)[0]
        l_carbon_normal = svd[:,-1]
        if l_carbon_normal[2] < 0.0:
            l_carbon_normal = -l_carbon_normal

        # Calculate the left contact angle
        cosine = np.dot(l_inter_normal, l_carbon_normal)
        cosine /= np.linalg.norm(l_inter_normal)
        cosine /= np.linalg.norm(l_carbon_normal)
        angles[azi] = np.arccos(cosine) * 180.0 / np.pi

        # Find first guess of right interface
        ...
        if np.max(densities) < CUTOFF_DENSITY:
            warnings.warn('Droplet too sparse, could not find interface', RuntimeWarning)
        lower = testpoints[np.argmax(densities), 0]
        upper = x_max
        result = (lower + upper) / 2.0
        while (upper - lower) > DISTANCE_TOLERANCE:
        ...
        # Use first guess to find reasonable water floor and establish scanning z-coordinates
        lowest_scan_z = (np.min(r_waters[np.abs(r_waters[:,0] - result) < SLICING_WIDTH, 2]) +
                         COARSE_GRAIN_LENGTH)
        z_coords = np.linspace(lowest_scan_z, highest_scan_z, N_SAMPLE_Z)
            ...
            for z_sample in z_coords:
                ...
                while (upper - lower) > DISTANCE_TOLERANCE:
                    ...
                    if density > CUTOFF_DENSITY:
                        ...
                        upper = result
                    result = (lower + upper) / 2.0
                r_interface.append([result, y_sample, z_sample])
        
        # Calculate normal vector of right interface
        r_interface = np.array(r_interface)
        r_inter_centroid = np.mean(r_interface, axis=0)
        svd = np.linalg.svd((r_interface - r_inter_centroid).T)[0]
        r_inter_normal = svd[:,-1]
        if r_inter_normal[0] < 0.0:
            r_inter_normal = -r_inter_normal
        
        # Find closest carbon atoms to the right interface, and calculate their normal vector
        r_inter_foot = np.zeros((3,), dtype=float)
        r_inter_foot[0] = r_inter_centroid[0] + (r_inter_centroid[2] * r_inter_normal[2] /
                                                 r_inter_normal[0])
        carbon_dists_sq = np.sum(np.square(carbon_slice - r_inter_foot), axis=-1)
        closest_carbons = carbon_slice[np.argpartition(carbon_dists_sq, 20)[:20]]
        r_carbon_centroid = np.mean(closest_carbons, axis=0)
        svd = np.linalg.svd((closest_carbons - r_carbon_centroid).T)[0]
        r_carbon_normal = svd[:,-1]
        if r_carbon_normal[2] < 0.0:
            r_carbon_normal = -r_carbon_normal

        # Calculate the right contact angle
        cosine = np.dot(r_inter_normal, r_carbon_normal)
        cosine /= np.linalg.norm(r_inter_normal)
        cosine /= np.linalg.norm(r_carbon_normal)
        ...
        if (fig is not None) and (azi < len(axes)):
            ...
            plot_x_min = 1.5 * l_inter_centroid[0]
            plot_x_max = 1.5 * r_inter_centroid[0]
            ...
            # Plot carbon planes
            point_a_x = l_carbon_centroid[0] - (0.1 * plot_width)
            point_a_z = l_carbon_centroid[2] + (0.1 * plot_width * l_carbon_normal[0] / l_carbon_normal[2])
            point_b_x = l_carbon_centroid[0] + (0.1 * plot_width)
            point_b_z = l_carbon_centroid[2] - (0.1 * plot_width * l_carbon_normal[0] / l_carbon_normal[2])
            axes[azi].plot((point_a_x, point_b_x), (point_a_z, point_b_z), 'k-')
            point_a_x = r_carbon_centroid[0] - (0.1 * plot_width)
            point_a_z = r_carbon_centroid[2] + (0.1 * plot_width * r_carbon_normal[0] / r_carbon_normal[2])
            point_b_x = r_carbon_centroid[0] + (0.1 * plot_width)
            point_b_z = r_carbon_centroid[2] - (0.1 * plot_width * r_carbon_normal[0] / r_carbon_normal[2])
            axes[azi].plot((point_a_x, point_b_x), (point_a_z, point_b_z), 'k-')

            # Plot water interfaces
            point_b_z = 2.0 * l_inter_centroid[2]
            point_b_x = l_inter_centroid[0] - (l_inter_centroid[2] * l_inter_normal[2] / l_inter_normal[0])
            axes[azi].plot((l_inter_foot[0], point_b_x), (0.0, point_b_z), '-', color=(1.0, 0.0, 1.0))
            point_b_z = 2.0 * r_inter_centroid[2]
            point_b_x = r_inter_centroid[0] - (r_inter_centroid[2] * r_inter_normal[2] / r_inter_normal[0])
...
#-----------------------
# Program mode (1):  Contact angle is measured as an instantaneous 'snapshot' from the last frame
# of the file. The angle is averaged over azimuthal slices. The program also generates a plot of
# the fit for each azimuthal slice over the water density if N_azimuths is less than 7; otherwise
# the plot is generated only for the first azimuthal slice.
...
if prog_args.N_frames == 1:
    ...
    time_start = time.time()
    if prog_args.opt_save or prog_args.opt_display:
        angles, highest_density = contact_angles(waters, carbons, cell_params, prog_args.N_azimuths,
